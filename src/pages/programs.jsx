      ...
      <div className="py-8 px-4 mx-auto max-w-screen-xl sm:py-16 lg:px-6">
        <div className="max-w-screen-md mb-8 lg:mb-16">
          ...
          <p className="text-gray-500 sm:text-xl dark:text-gray-400">
            Here you can find all the resources and details of Community
        ...
        <div className="space-y-8 md:grid md:grid-cols-2 lg:grid-cols-3 md:gap-12 md:space-y-0">
          {programs.map((program, index) => {
            if (program.end_date > new Date().toISOString().split("T")[0]) {
              return (
                <Link key={program.url}  href={program.url}>
                  <a target="_blank" >

                    <div
                      key={index}
                      className={`${ index > 0 ? 'mt-5' : ''} lg:mt-0 max-w-sm flex h-full flex-col bg-white 
