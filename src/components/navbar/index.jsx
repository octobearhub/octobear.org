    ...
    return (
        <nav className="bg-white  border-gray-200 px-2 sm:px-4 py-2.5 dark:bg-gray-900">
            <div className="h-[10vh] container flex flex-wrap justify-between items-center mx-auto">
                <Link href="/">
                    <a className="flex">
                        { !darkMode ? <Image src="/LOGO.svg" width="33" height="32" className="mr-3 h-6 sm:h-9" alt="Octobear Logo" /> : <Image src="/LOGO-black.svg" width="33" height="32" className="mr-3 h-6 sm:h-9" alt="Octobear Logo" />}
                        <span className="self-center text-xl font-semibold whitespace-nowrap dark:text-white">Octobear Hub</span>
                    </a>
                </Link>
                <button id="theme-toggle" onClick={toggleDarkMode} type="button" className=" inline-flex items-center  md:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600 text-gray-500   rounded-lg text-sm p-2.5">
                    <FaMoon id="theme-toggle-dark-icon" className={`${darkMode === false ? 'hidden' : '' }  justify-center items-center w-5 h-5`}/>
                    <FaSun id="theme-toggle-light-icon" className={`${darkMode === true ? 'hidden' : '' }  justify-center items-center w-5 h-5`}></FaSun>
                </button>
                ...
                <ul id="menu" className="flex flex-col p-4 mt-4 bg-gray-50 rounded-lg border border-gray-100 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
                    {LinksData?.map((link, index) => (
                        <li className="flex items-center" key={index}>
                            <Link href={link.url}>
                                <a className="block py-2 pr-4 pl-3  text-gray-900 bg-blue-700 rounded md:bg-transparent md:p-0 dark:text-white" aria-current="page">{link.title}</a>
                            </Link>
                        </li>
                    ))}
                    <li className='pr-4'>
